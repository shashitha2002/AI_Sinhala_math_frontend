...
const MathGame: React.FC = () => {
    ...
    const [userQuestions, setUserQuestions] = useState<UserQuestionResult[]>([]); // To store log of answered questions
    ...
    const handleSubmit = () => {
        ...
        if (isCorrect) {
            ...
            setFeedback({ type: 'incorrect', message: t('games.incorrect') || 'Incorrect!' });
        }
        ...
        return (
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
                <div className="bg-gradient-to-br from-primary-500 to-primary-700 rounded-2xl shadow-2xl p-6 sm:p-8 text-white text-center">
                    ...
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 text-white">{t('games.quickMath') || 'Quick Math'}</h1>
                    <p className="text-base sm:text-lg md:text-xl mb-6 sm:mb-8 opacity-90">
                        {t('games.playGame') || 'Play Game'} - 60 {t('games.seconds') || 'seconds'} {t('games.challenge') || 'challenge'}
                    </p>
                    <Button
                        onClick={startGame}
                        variant="secondary"
                ...
                <div className="bg-white dark:bg-dominant-800 rounded-2xl shadow-xl p-4 sm:p-8 text-center transition-colors">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-8">
                        <div className="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-4 sm:p-6">
                            ...
                            <p className="text-2xl sm:text-3xl font-bold text-primary-600 dark:text-primary-400">{score}</p>
                        </div>
                        ...
                        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4 sm:p-6">
                            ...
                            <p className="text-2xl sm:text-3xl font-bold text-purple-600 dark:text-purple-400">{userQuestions.length}</p>
                        </div>
                    </div>
                    <Button
                        onClick={startGame}
            ...
            <div className="bg-white dark:bg-dominant-800 rounded-2xl shadow-xl p-4 sm:p-8 transition-colors">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 sm:mb-6">
                    <div className="flex items-center gap-2 sm:gap-4">
                        <div className="bg-primary-100 dark:bg-primary-900/30 rounded-lg p-2 sm:p-3">
                            <TrophyIcon className="h-5 w-5 sm:h-6 sm:w-6 text-primary-600 dark:text-primary-400" />
                        </div>
                        <div>
                            <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">{t('games.yourScore') || 'Your Score'}</p>
                            <p className="text-xl sm:text-2xl font-bold text-primary-600 dark:text-primary-400">{score}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-4">
                ...
                {/* Question */}
                <div className="bg-gradient-to-r from-primary-50 to-purple-50 dark:from-primary-900/20 dark:to-purple-900/20 rounded-xl p-6 sm:p-12 text-center mb-4 sm:mb-6">
                    {feedback && (
                    ...
                    {!feedback && (
                        <>
                            <p className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6 break-words">
                            ...
                            <input
                                ...
                                className="text-2xl sm:text-3xl md:text-4xl font-bold text-center border-4 border-primary-300 dark:border-primary-700 rounded-lg px-4 sm:px-6 py-3 sm:py-4 focus:outline-none focus:border-primary-500 bg-white dark:bg-dominant-700 text-gray-900 dark:text-gray-100 w-full max-w-[200px] sm:w-48 touch-manipulation"
                                autoFocus
                                placeholder="?"
                                inputMode="numeric"
                            />
