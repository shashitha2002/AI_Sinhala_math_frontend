...
const DIFFICULTY_BADGE: Record<string, { label: string; labelEn: string; bg: string; text: string }> = {
    easy: { label: "පහසු", labelEn: "Easy", bg: "bg-green-100 dark:bg-green-900/40", text: "text-green-700 dark:text-green-300" },
    medium: { label: "මධ්‍යම", labelEn: "Medium", bg: "bg-yellow-100 dark:bg-yellow-900/40", text: "text-yellow-700 dark:text-yellow-300" },
    hard: { label: "අපහසු", labelEn: "Hard", bg: "bg-red-100 dark:bg-red-900/40", text: "text-red-700 dark:text-red-300" },
};
...
const QuizGenerator: React.FC<QuizGeneratorProps> = ({ user }) => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            ...
            try {
                ...
                if (mathResponse.topics && mathResponse.topics.length > 0) {
                    setMathTopicsData(mathResponse.topics);
                    setFormData((prev) => ({ ...prev, topic: mathResponse.topics[0].sinhala }));
                } else {
                    applyFallbackTopics();
                }
            } catch (err) {
    ...
    const applyFallbackTopics = () => {
        const fallback: MathTopic[] = [
        ...
        setMathTopicsData(fallback);
        setFormData((prev) => ({ ...prev, topic: fallback[0].sinhala }));
    };
    ...
    const handleGenerateMathQuiz = () => {
        ...
        navigate("/math-quiz", {
            state: {
                topic: formData.topic || mathTopicsData[0]?.sinhala || "පොළිය",
    ...
    const diffBadge = DIFFICULTY_BADGE[formData.difficulty] || DIFFICULTY_BADGE.medium;
            ...
            <div className="card shadow-xl p-4 sm:p-8">
                ...
                <div className="mb-6 p-4 bg-dominant-50 dark:bg-dominant-800 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select id="difficulty" name="difficulty" value={formData.difficulty} onChange={handleChange} className="input">
                        ...
                        <div>
                            ...
                            <select id="numQuestions" name="numQuestions" value={formData.numQuestions} onChange={handleChange} className="input">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
                    ...
                    <div className="card border-2 border-dashed border-blue-300 dark:border-blue-700 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30 hover:shadow-lg transition-all">
                        ...
                        <ul className="text-xs text-dominant-500 dark:text-dominant-500 mb-4 space-y-1">
                            {[
                                ...
                                "මාතෘකා 6ක් සහ අපහසුතා මට්ටම් 3ක් (6 topics, 3 difficulty levels)",
                            ].map((item, i) => (
                        ...
                        <div className="mb-4 p-2 bg-white dark:bg-dominant-900 rounded text-xs text-dominant-600 dark:text-dominant-400 flex flex-wrap items-center gap-2">
                            <span className="font-medium">Selected:</span>
                            <span className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                                {formData.topic}
                            </span>
                            <span className={`${diffBadge.bg} ${diffBadge.text} px-2 py-0.5 rounded`}>
                                {diffBadge.label}
                            </span>
                            <span className="bg-gray-100 dark:bg-dominant-700 px-2 py-0.5 rounded">
                                {formData.numQuestions} questions
                            </span>
                        </div>

                        <Button onClick={handleGenerateMathQuiz} variant="primary" disabled={loading || loadingTopics} fullWidth>
                            {loading ? (
                    ...
                    <div className="card border-2 border-dashed border-accent-300 dark:border-accent-700 bg-gradient-to-br from-accent-50 to-accent-100 dark:from-accent-900/30 dark:to-accent-800/30 hover:shadow-lg transition-all">
                        ...
                        <Button onClick={handleNavigateModelPaper} variant="primary" disabled={loading} fullWidth>
                            {loading ? (
                                <span className="flex items-center justify-center">
                                    <svg className="animate-spin -ml-1 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24">
                                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
...
// Quiz History Component (unchanged from your original)
    ...
    return (
        <div className="space-y-2">
            {history.slice(0, 5).map((quiz, idx) => (
                <div key={idx} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 bg-dominant-50 dark:bg-dominant-900 rounded-lg hover:bg-dominant-100 dark:hover:bg-dominant-800 transition-colors border border-dominant-200 dark:border-dominant-700">
